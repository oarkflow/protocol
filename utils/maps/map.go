...
type (
	...
	metadata[K hashable, V any] struct {
		...
		// use a struct element with generic params to enable monomorphization (generic code copy-paste) for the parent metadata struct by golang compiler leading to the best performance (truly hax)
...
// CompareAndSwap atomically updates a map entry given it's key by comparing current value to `oldValue`
...
func FromMap(data map[string]any) IMap[string, any] {
	mp := New[string, any]()
	for k, v := range data {
		mp.Set(k, v)
	}
...
func ToMap(data IMap[string, any]) map[string]any {
	mp := make(map[string]any)
	data.ForEach(func(s string, a any) bool {
		mp[s] = a
		return true
	})
	return mp
